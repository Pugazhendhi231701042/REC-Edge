...
export default function MasterAdminDashboard() {
  const [activeTab, setActiveTab] = useState<'departments' | 'users' | 'regulations' | 'popso' | 'audit'>('departments');
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [resDepts, resUsers, resRegs, resLogs] = await Promise.all([
  ...
  const handleSavePOPSOConfig = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setPoMsg('PO / PSO Configuration updated successfully for department.');
      }
    } catch (err) {
      ...
      <div className="space-y-8 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-xs text-desc mt-1">Manage Departments, Users, Regulations, and Programme-Level Configurations</p>
          </div>
        </div>
        ...
        {/* TAB 2: USERS & PASSWORD RESET (Rule 11, 15: Never display plaintext passwords) */}
        {activeTab === 'users' && (
          <div className="bg-white rounded-2xl border border-purple-100 p-6 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-base font-bold text-slate-900">User Directory</h3>
                <p className="text-xs text-desc">All user accounts. MasterAdmin can trigger secure password resets.</p>
              </div>
              <button
                onClick={() => {
                  setUserEmail('');
                    ...
                    <tr key={u.id} className="hover:bg-slate-50">
                      ...
                      <td className="p-3 text-right">
                        <button
                          ...
                          className="px-3 py-1 bg-amber-50 hover:bg-amber-100 text-amber-800 font-bold rounded-lg border border-amber-200 text-xs flex items-center inline-flex"
                        >
        ...
        {activeTab === 'regulations' && (
          <div className="bg-white rounded-2xl border border-purple-100 p-6 shadow-sm space-y-4">
            ...
            <p className="text-xs text-desc">Configure institution regulations. Regulation 26 is currently active.</p>

            <div className="space-y-3">
              {regulations.map((r) => (
                <div key={r.id} className="p-4 border rounded-xl flex items-center justify-between bg-slate-50">
                  <div>
        ...
        {/* TAB 4: PO / PSO CONFIGURATION */}
        {activeTab === 'popso' && (
          <div className="bg-white rounded-2xl border border-purple-100 p-6 shadow-sm space-y-5 max-w-xl">
            <h3 className="text-base font-bold text-slate-900">PO & PSO Count Configuration</h3>
            <p className="text-xs text-desc">
              Institution rules mandate <strong>12 POs</strong> and <strong>3 PSOs</strong> by default.
            </p>
            ...
            <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
              ...
              <form onSubmit={handleSaveDepartment} className="space-y-3 text-xs">
                <div>
                  ...
                  <select value={programmeType} onChange={(e) => setProgrammeType(e.target.value)} className="w-full p-2 border rounded-lg">
                ...
                <div>
                  ...
                  <input type="text" required value={programmeName} onChange={(e) => setProgrammeName(e.target.value)} placeholder="Computer Science and Engineering" className="w-full p-2 border rounded-lg" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block font-semibold mb-1">Short Name *</label>
                    <input type="text" required value={shortName} onChange={(e) => setShortName(e.target.value)} placeholder="CSE" className="w-full p-2 border rounded-lg" />
                  </div>
                  <div>
                    <label className="block font-semibold mb-1">Department Code *</label>
                    <input type="text" required value={departmentCode} onChange={(e) => setDepartmentCode(e.target.value)} placeholder="CS" className="w-full p-2 border rounded-lg uppercase" />
                  </div>
                </div>
                <div>
                  <label className="block font-semibold mb-1">Number of Semesters *</label>
                  <input type="number" required min="1" max="10" value={semesters} onChange={(e) => setSemesters(parseInt(e.target.value) || 8)} className="w-full p-2 border rounded-lg" />
                </div>
                <div>
                  <label className="block font-semibold mb-1">Assign Head of Department (HoD)</label>
                  <select value={hodId} onChange={(e) => setHodId(e.target.value)} className="w-full p-2 border rounded-lg">
                ...
                <div className="flex justify-end space-x-2 pt-3 border-t">
                  <button type="button" onClick={() => setShowDeptModal(false)} className="px-3 py-1.5 rounded-lg text-slate-600">Cancel</button>
                  <button type="submit" className="px-4 py-1.5 rounded-lg bg-brand-600 text-white font-bold">Save Department</button>
                </div>
              </form>
            </div>
          </div>
            ...
            <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
              ...
              <form onSubmit={handleCreateUser} className="space-y-3 text-xs">
                <div>
                  ...
                  <input type="text" required value={userName} onChange={(e) => setUserName(e.target.value)} placeholder="Dr. Jane Doe" className="w-full p-2 border rounded-lg" />
                </div>
                <div>
                  <label className="block font-semibold mb-1">College Email (@rajalakshmi.edu.in) *</label>
                  <input type="email" required value={userEmail} onChange={(e) => setUserEmail(e.target.value)} placeholder="name@rajalakshmi.edu.in" className="w-full p-2 border rounded-lg" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block font-semibold mb-1">Role *</label>
                    <select value={userRole} onChange={(e) => setUserRole(e.target.value)} className="w-full p-2 border rounded-lg">
                  ...
                  <div>
                    ...
                    <select value={userDeptId} onChange={(e) => setUserDeptId(e.target.value)} className="w-full p-2 border rounded-lg">
                ...
                <div>
                  ...
                  <input type="password" required value={userPassword} onChange={(e) => setUserPassword(e.target.value)} className="w-full p-2 border rounded-lg" />
                </div>
                <div className="flex justify-end space-x-2 pt-3 border-t">
                  <button type="button" onClick={() => setShowUserModal(false)} className="px-3 py-1.5 rounded-lg text-slate-600">Cancel</button>
                  <button type="submit" className="px-4 py-1.5 rounded-lg bg-brand-600 text-white font-bold">Create Account</button>
                </div>
              </form>
            </div>
          </div>
        ...
        {/* Reset Password Modal (Rule 11, 15: MasterAdmin NEVER sees existing plaintext password) */}
        {showResetModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
              ...
              <form onSubmit={handleResetUserPassword} className="space-y-3 text-xs">
                <div>
                  <label className="block font-semibold mb-1">New Hashed Password *</label>
                  <input type="password" required value={resetPassword} onChange={(e) => setResetPassword(e.target.value)} className="w-full p-2 border rounded-lg" />
                </div>
                <div className="flex justify-end space-x-2 pt-3 border-t">
                  <button type="button" onClick={() => setShowResetModal(false)} className="px-3 py-1.5 rounded-lg text-slate-600">Cancel</button>
                  <button type="submit" className="px-4 py-1.5 rounded-lg bg-amber-600 text-white font-bold">Reset Password</button>
                </div>
              </form>
            </div>
          </div>
